<?php
// Script to correct break record association
session_start();
...
// Update break record ID 125 to associate with time record ID 202
$stmt = $time_db->prepare("UPDATE breaks SET time_record_id = 202 WHERE id = 125");
if ($stmt->execute()) {
    echo 'Break record successfully updated to associate with time record ID 202.';
} else {
